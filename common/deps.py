...
def check_jwt_token(token: Optional[str] = Header(..., description='登录token')) -> Union[str, Any]:
    """
...
def get_current_user(
    db: Session = Depends(get_db), token: Optional[str] = Depends(check_jwt_token)
) -> SysUser:
    """
...
def check_authority(request: Request, token: Optional[str] = Depends(check_jwt_token)):
    """
    权限验证 依赖于 JWT token
    :param request:
    :param token:
