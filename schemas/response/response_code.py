...
def resp_500(
    *, data: Union[list, dict, str] = None, message: str = 'Internal Server Error'
) -> Response:
    return JSONResponse(
...
def resp_4001(
    *,
    ...
    message: Union[list, dict, str] = 'Request Validation Error'
) -> Response:
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content=jsonable_encoder(
...
# 内部验证数据错误
def resp_5002(
    *, data: Union[list, dict, str] = None, message: Union[list, dict, str] = 'Request Fail'
) -> Response:
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content=jsonable_encoder(
